Shader "Custom/SnowTracks_normals"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        float3 filterNormalBlue(sampler2D tex, float2 uv, float texelSize, float texelAspect) 
        { 
            float4 h; 

            h[0] = tex2Dlod(tex, float4(uv + texelSize * float2( 0,-1),0,0)).b * texelAspect; 
            h[1] = tex2Dlod(tex, float4(uv + texelSize * float2(-1, 0),0,0)).b * texelAspect; 
            h[2] = tex2Dlod(tex, float4(uv + texelSize * float2( 1, 0),0,0)).b * texelAspect; 
            h[3] = tex2Dlod(tex, float4(uv + texelSize * float2( 0, 1),0,0)).b * texelAspect;
        ...
        void disp (inout appdata v)
        {
            ...
            if(depthCoordinate.r == 0 && depthCoordinate.b == 0) {
                v.normal = filterNormal(_PerlinTex, v.texcoord.xy, (float)((float)1/(float)256), 256);
            } else if (depthCoordinate.r > 0) {
                v.normal = filterNormal(_Splat, v.texcoord.xy, (float)((float)1/(float)1024), 1024);
            } else if (depthCoordinate.b > 0) {
                v.normal = filterNormalBlue(_Splat, v.texcoord.xy, (float)((float)1/(float)1024), 1024);
            }
        ...
        void surf (Input IN, inout SurfaceOutput o)
        {
            ...
            if(amount > 1 - _textureDepthOffset) {
                ...
                float3 normals = UnpackNormal( tex2D(_SnowNormalMap, IN.uv_SnowNormalMap) );
                float3 detailedNormals = UnpackNormal( tex2D(_DetailedSnowMap, IN.uv_DetailedSnowMap) );
                float3 snowNormals =  BlendNormals(normals, detailedNormals);
                o.Normal = snowNormals;
